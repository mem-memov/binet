...
class ToBigEndianSuite extends munit.FunSuite:
  ...
  test("Address gets converted to an array of bytes in big-endian order") {

    val address = Address(List(b3, b2, b1, b0))

    val result = address.toBigEndian

    assert(result sameElements Array[Byte](-128, -127, -126, -125))
  }
